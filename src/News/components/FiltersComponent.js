...
const {
    ...
    constants: { categories, languages, countries }
} = Common;
...
class FiltersComponent extends Component {
    ...
    handleFilter(target, type) {
        if (target.value === categories.SELECT_CATEGORY ||
            target.value === countries.SELECT_COUNTRY ||
            target.value === languages.SELECT_LANGUAGE) {
            return;
        }
        this.setState({
            [type]: target.value
            ...
            <aside className={css(styles.formWrapper)}>
                ...
                <form onSubmit={handleSubmit} className={css(styles.formStyle)}>
                    ...
                    <div>
                        <ul>
                            ...
                            filters.category.map((value, index) =>
                                <li key={index}>{value}
                                    <span className={css(styles.deleteButton)}
                                          onClick={() => this.handleRemove('category', value)}>
                                        x
                                    </span>
                                </li>)}
                        </ul>
                    ...
                    <div>
                        <ul>
                            {filters.language &&
                            filters.language.map((value, index) =>
                                <li key={index}>{value}
                                    <span className={css(styles.deleteButton)}
                                          onClick={() => this.handleRemove('language', value)}>
                                        x
                                    </span>
                                </li>)}
                        </ul>
                    ...
                    <div>
                        <ul>
                            {filters.country &&
                            filters.country.map((value, index) =>
                                <li key={index}>{value}
                                    <span className={css(styles.deleteButton)}
                                          onClick={() => this.handleRemove('country', value)}>
                                        x
                                    </span>
                                </li>)}
                        </ul>
