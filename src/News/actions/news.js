        ...
        return getDataAction(url)
            .then((d) => {
                dispatch(actionCreator(
                    actionType.SUCCESS_GET_PARTICULAR_NEWS,
                    {articles: d.articles, id})
                );
                dispatch(actionCreator(actionType.REQUEST_END));
                return Promise.resolve(d.articles);
            })
            .catch((error) => {
                dispatch(actionCreator(actionType.REQUEST_END));
                return Promise.reject(error);
...
export function getNewsFromLocalStorage(news) {
    return (dispatch) => {
        dispatch(actionCreator(actionType.SUCCESS_GET_NEWS, {news}));
    ...
    return (dispatch) => {
        ...
        return getDataAction(url)
            .then((data) => {
                dispatch(actionCreator(actionType.REQUEST_BEGIN));
                dispatch(actionCreator(
                    actionType.SUCCESS_GET_SORTED_ARTICLES,
                    {articles: data.articles, id, sortBy})
                );
