            ...
            <ul className={css(styles.pagination)}>
                {this.pageIndexesList(size).map((item, index) => (
                    <li key={index}
                        ...
                        ` ${(index === 0 && css(styles.firstItem)) || null}
                             ${(active === item && css(styles.active)) || null}`}>
                            <span
                                onClick={() => this.makeActive(item)}
                                className={(!this.show(index + 1, size).index &&
                                    css(styles.hidden)) ||
                                css(styles.common)}>{item}</span>
                        <span
                            className={(!this.show(index + 1, size).ellipsis &&
                                css(styles.hidden)) ||
                            css(styles.common)}>
                                ...
                            </span>
