...
const {
    components: { NewsItem, ArticleComponent },
    helpers: { removeArticleFromStorage,
...
class NewsPageComponent extends React.Component {
    ...
    componentDidMount() {
        ...
        if (!data) {
            ...
            if (!element) {
                ...
                const isInStore = Object.keys(this.props.articles).length &&
                    this.props.articles[news.id] &&
                    this.props.articles[news.id].latest.length;
    ...
    filterArticleById(art, id, type) {
        const filterType = type;
        const isExist = art[id];
        if (Object.keys(art).length === 0 || !isExist) {
            return [];
        }
        this.setState({
            view: art[id][filterType]
        });
    }
    getParticularNews(item) {
        ...
        if (!element && !elementInStore) {
            this.props.getParticularNews(item, item.id)
                .then((d) => {
                    saveArticlesToStorage(d, item.id, 'latest');
        ...
        if (type === 'top') {
            if (element) {
                ...
                this.props.getSortedArticles(this.state.id, 'top')
                    .then((d) => {
                        saveArticlesToStorage(d, this.state.id, 'top');
        ...
        if (type === 'popularity') {
            if (element) {
                ...
                this.props.getSortedArticles(this.state.id, 'popularity')
                    .then((d) => {
                        saveArticlesToStorage(d, this.state.id, 'popularity');
    ...
    joinNews() {
        return this.props.location.state.filteredNews.reduce(
                ...
                <div style={{flex: 4}}>
                    ...
                    {(view && view.length &&
                        <ArticleComponent
                            articles={view}
                        />) ||
                        <p className={css(styles.noarticles)}>
